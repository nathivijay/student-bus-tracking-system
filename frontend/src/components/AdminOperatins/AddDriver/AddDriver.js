...
const AddDriver = () => {
  ...
  const validateForm = () => {
    let isValid = true;
    const newErrors = { name: "", pinNo: "", busNo: "" };

    if (!formData.name) {
      newErrors.name = "Name PIN is required";
      isValid = false;
    }

    if (!formData.pinNo) {
      newErrors.pinNo = "Password is required";
      isValid = false;
    } else if (formData.pinNo.length < 6) {
      newErrors.pinNo = "Password must be at least 6 characters";
      isValid = false;
    }

    if (!formData.busNo) {
      newErrors.busNo = "Password is required";
      isValid = false;
    }
  ...
  return (
    <>
      <Popup
        modal
        trigger={<button className="add-student-button">Add Driver</button>}
      >
        {(close) => (
          <>
            <div className="modal-popup">
              ...
              <h2 className="modal-heading">Add New Driver</h2>
